  ...
  for strLine in strLines:
    ...
    if "=" in strLine:
      ...
      if strVarName == "FullFromDate":
        dtFullLoad = strValue
      if strVarName == "TestQID":
        if isInt(strValue):
          iTestQID = int(strValue)
        else:
          iTestQID = "-10"
...
def UpdateDB (dictResults):
  ...
  if not ValidReturn(lstReturn):
    ...
    LogEntry ("Not in tbldetections")
    return
  if dictResults["QID"] in lstProcessedQIDs:
    LogEntry ("Already Processed")
    return
  else:
    lstProcessedQIDs.append(dictResults["QID"])
...
if strLoadType.lower() == "full":
  ...
  strSQL = "select distinct iQID from tbldetections;"
elif strLoadType.lower() == "test" :
  LogEntry("Per configuration file doing a test load focusing on QID {}".format(iTestQID))
  strSQL = "select distinct iQID from tbldetections where iQID = {};".format(iTestQID)
elif strLoadType.lower() == "last" :
  LogEntry("Per configuration file updating all recoreds not updated since {}".format(dtLastExecute))
  strSQL = "select iQID from tblVulnDetails where dtLastTouched < '{}';".format(dtLastExecute)
else:
  LogEntry("Per configuration file doing an incremental load")
  strSQL = "select distinct iQID from tbldetections where iQID not in (select iQID from tblVulnDetails);"

LogEntry ("Fetching list of QID's from the database")
lstQIDs = SQLQuery (strSQL,dbConn)
if not ValidReturn(lstQIDs):
  LogEntry ("Unexpected: {}".format(lstQIDs))
  CleanExit("due to unexpected SQL return, please check the logs")
else:
  LogEntry ("Fetched {} rows".format(len(lstQIDs[1])))

iTotalRows = len(lstQIDs[1])
iRowNum = 1
iUpdateCount = 0
lstProcessedQIDs = []
...
for dbRow in lstQIDs[1]:
  dictParams["ids"] = dbRow[0]
  LogEntry("\n--------------\nWorking on QID: {} record {} out of {}. {:.1%} complete".format(
            dbRow[0],iRowNum,iTotalRows,(iRowNum-1)/iTotalRows))
...
if strLoadType.lower() != "full" and strLoadType.lower() != "test" and dtLastExecute != -10 :
  LogEntry ("Checking for QID's updated since {}.".format(dtLastExecute))
  if "ids" in dictParams:
    del dictParams["ids"]
  dictParams["last_modified_by_service_after"] = dtLastExecute
  strListScans = urlparse.urlencode(dictParams)
  strURL = strBaseURL + strAPIFunction +"?" + strListScans
  APIResponse = MakeAPICall(strURL,strHeader,strUserName,strPWD,strMethod)
  ProcessResponse(APIResponse)
